...
function Dashboard() {
    ...
    const boxes = [
        {name: "Customers", icon: <i className="fa-solid fa-people-group"></i>, amount: 0, backgroundColor: "#41B06E"},
        {name: "Orders", icon: <i className="fa-solid fa-cart-shopping"></i>, amount: 0, backgroundColor: "#1679AB"},
    ];
    ...
    return (
        <div className="flex-1 flex flex-col gap-5 text-white mb-16">
            ...
            <div className="flex flex-col md:flex-row gap-5">
                {boxes.map((box, index) => (
                    ...
                    key={index}>
                        ...
                        <p className="text-5xl w-full pt-4 flex justify-end font-medium">{box.amount}</p>
                    </div>
                ))}
            </div>
            <div className="flex-1">
                ...
                <div className="flex flex-col gap-5">
                    <div className="flex flex-col shadow-md shadow-gray-400 rounded-lg p-4 bg-[#F6B17A]">
                        <p className="text-md font-bold flex justify-between"><span>Total Amount of Products: </span><span>0</span></p>
                    </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {inventory.map((brand, index) => (
                                ...
                                >
                                    ...
                                    <p className="text-gray-600 text-sm w-full pt-2 flex justify-between font-medium"><span>Total Products:</span><span>{brand.totalAmount}</span></p>
                                    <p className="text-gray-600 text-sm w-full pt-2 flex justify-between font-medium"><span>Variety:</span><span>{brand.variety}</span></p>
                                </div>
                            ))}
                        </div>
                    </div>  
